...
class LightCLIControlProgram(BaseProgram):
    ...
    def add_opts(self):
        ...
        self.opt_parser.add_argument(
            '-b', '--brightness',
            dest='bri', type=self.int_within_range(1, 254),
            help='set brightness (1 to 254)')
        self.opt_parser.add_argument(
            '-u', '--hue',
            dest='hue', type=self.int_within_range(0, 65535),
            help='set hue (0 to 65535)')
        self.opt_parser.add_argument(
            '-s', '--saturation',
            dest='sat', type=self.int_within_range(1, 254),
        ...
        self.opt_parser.add_argument(
            '-c', '--ct', '--color-temp',
            dest='ct', type=self.int_within_range(1, 1e6),
            metavar='MIREDS',
            help='set color temperature in mireds/mireks')
        self.opt_parser.add_argument(
            '-k', '--kelvin',
            dest='ctk', type=self.int_within_range(1, 1e8),
            metavar='KELVIN',
            help='set color temperature in Kelvin')

        self.opt_parser.add_argument(
            '-i', '--incandescent',
            dest='incan', type=self.int_within_range(1, 254),
            metavar='BRI',
